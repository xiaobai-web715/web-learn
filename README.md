...
keepAlive 实现路由缓存 => 20230615(路由缓存好像会导致首屏数据加载不出来 => 目前不清楚是同一页面多次打开还是其他的原因 => 原因 [Table 的 created => list 的 mounted => Table 的 activated => list 的 activated] 但是到这里 Table 可能已经被缓存起来了导致数据没有更新到 (解决方法用 key 强制刷新一下 table)
...
记录 script 添加 lang="ts"报错缺少 loader 配置(最终原因不支持 ts 写法,要执行 vue add typescript 是框架具有编写 ts 的能力)
