...
//     // 要判断14是不是回文的话就要判断23是不是回文，所以23的判断要在14的前面
//     const len = s.length;
...
//                 dp[i][j] = false;
//             } else {
//                 if (i + 1 === j - 1 || j - i == 1) {  // 长度为3
//                     dp[i][j] = true;

//                 } else {
//                     dp[i][j] = dp[i+1][j-1]; // 长度为4及以上
//                 }
//             }
//             if (dp[i][j]) {
//                 const len = j - i + 1;
...
// console.log(longestPalindrome(''))

// function convert(s: string, numRows: number): string {
//     // 找规律的话 一个完整的部分是 n + n - 2组成一个循环
//     const unit : number = numRows + Math.max(0, numRows - 2); // 每一层所具有的成员数量  numRows + numRows - 2 - (i - 1) - 1 = 2 * numRows - i - 2
//     const layer : number = parseInt(String(s.length / unit)) + 1;
//     let result = '';
//     console.log("我是每次遍历的上限", unit, layer);
//     wrapper : for(let i = 0; i < numRows; i++) {
//         for(let j = 0; j < layer; j++) {
//             const index = i + j * unit; // 当前元素所处的index位置
//             if (index < s.length) {
//                 result += s[index];
//             } else {
//                 continue wrapper;
...
//             if (!(i == 0 || i == numRows - 1)) {
//                 // 此时还有一个斜边的值
//                 const index = (j + 1)(2 * numRows -i - 1) - 1;
...
// function convert(s: string, numRows: number): string {
//     // 找规律的话 一个完整的部分是 n + n - 2组成一个循环
//     const unit : number = numRows + Math.max(0, numRows - 2); // 每一层所具有的成员数量  numRows + numRows - 2 - (i - 1) - 1 = (2 * numRows - i - 2)  +  j * (numRows + (numRows - 2 - (i -1))) = 2numRows - i -2 + j (2 * numRows -i - 1) = (j + 1)(2 * numRows -i - 1) - 1
//     const layer : number = parseInt(String(s.length / unit)) + 1;
//     let result = '';
//     console.log("我是每次遍历的上限", unit, layer);
//     wrapper : for(let i = 0; i < numRows; i++) {
...
//             if (!(i == 0 || i == numRows - 1)) {
//                 // 此时还有一个斜边的值
//                 const index = 2 * (j + 1) * (numRows - 1) - i; // (numRows + numRows - 2 + 1 - i) + j(numRows + numRows - 2) - 1
//                 if (index < s.length ) {
//                     result += s[index];
//                 } else {
//                     continue wrapper;
...
//     const reg = /(^[+-]\d+)|(^\d\d*)/
//     const clearTrim = /(^[ ]+)(.*)/;
//     const noTrims = s.replace(clearTrim, "$2")
//     console.log("noTrims", noTrims)
//     const target = reg.exec(noTrims);
//     if (Array.isArray(target)) {
//         const result = target[0];
//         const numVal =  Number(result)
//         if (numVal < -(2 ** 31)) {
//             return -(2 ** 31)
//         } else if (numVal > 2 ** 31 - 1) {
//             return 2 ** 31 - 1
//         } else {
//             return numVal
//         }
...
// console.log(myAtoi("with words 1234"))


/* 本地用indexOf与while的方式能差30000ms左右,但使用indexOf在力扣上超时，while就不会导致超时 */
// function threeSum(nums: number[]): number[][] {
//     const startTime = new Date().valueOf();
//     // 对数组进行从小到大的排序处理
//     nums.sort((a, b) => a - b);
//     const result = [];
//     // 分为左右两个指针 （左边是第一个和第二个数，第二个数起点在第一个数索引的前边，第三个数保证在第二个数的前边，这样匹配第三个数的时候能保证只使用lastIndex即可）
...
//                     continue inner
//                 } else {
//                     const thirdNum = - nums[i] - nums[j]
//                     let indexOf = nums.lastIndexOf(thirdNum);
//                     if (indexOf > -1 && indexOf > j) {
//                         // 当前有第三个数的位置,并且第三个数的索引位置要比第二个数大
//                         result.push([nums[i], nums[j], nums[indexOf]]) 
//                     }
//                     /* 用indexOf的化时间过不去 */
//                     // if (thirdNum < nums[j]) {
//                     //     continue outer
//                     // }
//                     // let indexOf = nums.length - 1;
//                     // while(indexOf > j) {
//                     //     if (nums[indexOf] == thirdNum) {
//                     //         result.push([nums[i], nums[j], nums[indexOf]])
//                     //         continue inner;
//                     //     } else if (nums[indexOf] < thirdNum) {
//                     //         continue inner;
//                     //     } else {
//                     //         indexOf--
//                     //     }
...
//             continue outer
//         }
//         mid: for(let j = i + 1; j < nums.length; j++) {
...
//                         continue inner;
//                     } else {
//                         returnArray.push([nums[i], nums[j], nums[k], nums[l]])
//                         k--
//                         continue inner;
//                     }
//                 }
