<template>
  <div class="loginPage">
    <div class="useLogin">用户登录</div>
    <form class="loginForm">
      <label for="user">用户名:</label>
      <input id="user" v-model="formData.user" /><br />
    ...
    <div class="scrollModel">
      <div class="testModel">
        <div
          v-if="testSlidingModule.oriImageUrl"
          class="oriImage"
          :style="{
            backgroundImage: `url(${testSlidingModule.oriImageUrl})`,
            width: `${oriImageInfo?.width}px`,
            height: `${oriImageInfo?.height}px`,
          }"
        >
          <div
            v-if="testSlidingModule.newImageUrl"
            class="newImage"
            :style="{
              backgroundImage: `url(${testSlidingModule.newImageUrl})`,
              width: `${newImageInfo?.width + 2 * newImageInfo?.radius}px`,
              height: `${newImageInfo?.height + 2 * newImageInfo?.radius}px`,
              top: `${topPercent * 100}%`,
  ...
  mounted() {
    ...
    request({
      ...
      if (res && res.code == 200) {
        ...
        const oriImageUrlBlob = getBase64ToBLob(
          imageObj["oriImage"],
          "image/png"
        );
        const oriImageUrl = URL.createObjectURL(oriImageUrlBlob);
        const newImageUrlBlob = getBase64ToBLob(
          imageObj["newImage"],
          "image/png"
        );
